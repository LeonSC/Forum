	...
	<div class="row">
		...
		<div class="col-xs-10">
			...
			<div class="row">
				...
				<div class="col-xs-12">
					<c:if test="${empty forumtitle.background.src}"><img data-src="holder.js/1600x550" style="width:100%" id="background_BG_imgSrc_img"/></c:if>
					<c:if test="${not empty forumtitle.background.src}"><img src="http://${config.domainBucket}/${forumtitle.background.src}" style="width:100%" id="background_BG_imgSrc_img"/></c:if>
				</div>
				<div class="col-xs-12"><br/></div>
				<div class="col-xs-12">
				<form class="form-inline" action="${config.rootPath}/admin/background-background-submit" method="post">
					<div class="form-group">
						<input type="hidden" class="form-control" name="BM_ID" value="${forumtitle.BM_ID}"/>
						<input type="hidden" class="form-control" name="imgSrc" value="${forumtitle.icon}" id="background_BG_imgSrc"/>
					</div>
					<button type="button" class="btn btn-default" id="background_BG_imgSrc_button">选择</button>
					<button type="button" class="btn btn-default" onclick="document.getElementById('background_BG_imgSrc').value=''">取消</button>
					<button type="submit" class="btn btn-default">提交</button>
				</form>
				</div>
			</div>
...
$(document).ready(function(){
	...
	//背景图上传
	var qiniuBackground = new QiniuJsSDK();
	var uploaderBackground = qiniuBackground.uploader({
	    runtimes: 'html5,html4',
	    browse_button: 'background_BG_imgSrc_button',//上传选择的点选按钮，**必需**
	    uptoken_url: '${config.rootPath}/img/adminToken',//Ajax请求upToken的Url，**强烈建议设置**（服务端提供）
	    unique_names: true, // 默认 false，key为文件名。若开启该选项，SDK为自动生成上传成功后的key（文件名）。
	    domain: '${config.domainBucket}',   //bucket 域名，下载资源时用到，**必需**
	    get_new_uptoken: false,  //设置上传文件的时候是否每次都重新获取新的token
	    ...
	    init: {
	        ...
	        'BeforeUpload': function(up, file) {
				...
				$("#background_BG_imgSrc_button").attr("disabled",true);
	        },
	        'UploadProgress': function(up, file) {
	        	// 每个文件上传时,处理相关的事情
	        	////console.log(JSON.stringify(file));
	        ...
	        'FileUploaded': function(up, file, info) {
				...
				$.each(up.files,function(i,n){
					...
					//console.log(JSON.stringify(up));
					//console.log(JSON.stringify(info));
					$("#background_BG_imgSrc").val(n.target_name);
					$("#background_BG_imgSrc_img").attr("src","http://"+domain+'/'+n.target_name);
				});
	        },
	        'Error': function(up, err, errTip) {
	               //上传出错时,处理相关的事情
	        ...
	        'UploadComplete': function() {
	            //队列文件处理完毕后,处理相关的事情
	        	$("#background_BG_imgSrc_button").attr("disabled",false);
	        },
	        'Key': function(up, file) {
	            // 若想在前端对每个文件的key进行个性化处理，可以配置该函数
	            // 该配置必须要在 unique_names: false , save_key: false 时才生效
